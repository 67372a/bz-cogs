...
class EditImageToolCall(ImageToolCall):
    ...
    async def _handle(self, arguments: Dict[str, Any]) -> Union[str, List[Dict]]:
        ...
        try:
            ...
            logger.error(
                f"[EditImage] API call failed for guild {self.ctx.guild.name} after retries: {error_detail}"
            )
            return f"Error: Image edit API call failed after retries: {error_detail}"
        except Exception as e:
            logger.error(
                f"[EditImage] API call failed for guild {self.ctx.guild.name}: {type(e).__name__}: {e}"
            )
            return f"Error: Image edit API call failed: {type(e).__name__}: {str(e)}"
        ...
        content_parts.insert(0, {
            "type": "text",
            "text": (
                "Successfully edited the image. "
                "The edited image is attached to this function response and "
                "will be sent to the user alongside your reply."
            ),
        })
